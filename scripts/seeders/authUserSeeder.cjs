...
const TEST_USERS = [
	{
		...
		role: "admin",
		isActive: true,
		emailVerified: true, // Admin account can be pre-verified
		bio: "System administrator with full access to all features.",
		avatar: "https://ui-avatars.com/api/?name=Admin+User&background=0D8ABC&color=fff",
		social: {
			twitter: "@t2soft_admin",
			linkedin: "https://linkedin.com/in/t2soft-admin",
			github: "@t2soft",
		},
	},
	{
		...
		role: "student",
		isActive: true,
		emailVerified: false, // Require verification for regular users
...
async function createFirestoreUser(authUser, userData) {
	try {
		// Clean up social fields to avoid undefined values
		...
		const userDoc = {
			...
			social: {
				twitter:
					socialData.twitter ||
					(Math.random() < 0.3 ? `@${faker.internet.username()}` : null),
				linkedin:
					socialData.linkedin || (Math.random() < 0.3 ? faker.internet.url() : null),
				github:
					socialData.github ||
					(Math.random() < 0.2 ? `@${faker.internet.username()}` : null),
			},
			stats: {
